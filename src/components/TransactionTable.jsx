        ...
        <tbody>
          {transactions.map((transaction) => (
            <tr key={transaction.id}>
              ...
              <td className="py-2">{formatInvoiceNumber(transaction.invoiceNumber)}</td>
              <td className="py-2">{formatPhoneNumber(transaction.phoneNumber)}</td>
              <td className="py-2">{transaction.price}</td>
              <td className={`py-2 ${transaction.status === 'complete' ? 'text-lime-500' : 'text-yellow-500'}`}>
                {transaction.status}
              </td>
            </tr>
          ))}
