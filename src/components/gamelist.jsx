...
const GameList = () => {
  return (
    <div className="container mx-auto px-20 py-10"> {/* Add px-4 for padding on sides */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6"> {/* 5 columns on large screens */}
        {games.map((game, index) => (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 text-white p-2 md:p-4 opacity-0 translate-y-full group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-in-out">
              <h3 className="text-sm md:text-lg font-bold">{game.name}</h3>
              <p className="text-xs md:text-sm">{game.publisher}</p>
            </div>
          </div>
        ))}
      </div>
